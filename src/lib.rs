...
/// Object data is NOT stored in contract state — it is persisted in
/// the transaction history and retrieved via archival RPC.
...
impl GitStorage {
    ...
    /// Accept git objects and compute their SHA-1 hashes.
    ///
    /// Object data is NOT stored in contract state — it is persisted in
    /// the transaction args and can be retrieved from archival nodes.
    /// Only the computed SHAs are returned for use with `register_push`.
    ...
    ///
    /// - Stores SHA -> tx_hash mappings for archival retrieval
    ...
    /// Clients use the tx_hash to fetch object data from archival RPC.
